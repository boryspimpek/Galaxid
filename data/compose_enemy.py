...
LEVELS_FILE = "lvl17.json"  # Tu masz strukturę poziomów
...
def get_real_bank_id(enemy_shapebank, active_banks=None):
    """
    Tłumaczy indeks slotu wroga na fizyczny numer banku SHP.
    Jeśli active_banks (z jsona poziomu) jest dostępny, używa go.
    """
    if active_banks and 0 <= enemy_shapebank < len(active_banks):
        return active_banks[enemy_shapebank]
    
    # Fallback: jeśli nie mamy danych poziomu, zakładamy bezpośrednie mapowanie
    return enemy_shapebank

def shapebank_to_foldername(real_bank_id):
    """Mapuje fizyczny ID banku na nazwę folderu."""
    if 0 <= real_bank_id <= 9:
        return f"newsh{real_bank_id}.shp" # Usunąłem "extracted_" stąd dla testu
    elif 10 <= real_bank_id <= 35:
        letter = chr(ord('a') + (real_bank_id - 10))
        return f"newsh{letter}.shp"
    elif real_bank_id == 26:
        return "newsh~.shp"
    return f"newsh{real_bank_id}.shp"

def get_tiles_dir(real_bank_id):
    name = shapebank_to_foldername(real_bank_id)
    
    # Lista potencjalnych nazw folderów do sprawdzenia
    possible_names = [
        f"extracted_{name}", # np. extracted_newsh2.shp
        name,                # np. newsh2.shp
        name.replace(".shp", ""), # np. newsh2
        f"extracted_{name.replace('.shp', '')}" # np. extracted_newsh2
    ]
    
    for folder in possible_names:
        path = os.path.join(BASE_TILES_DIR, folder)
        if os.path.exists(path) and os.path.isdir(path):
            return path
            
    # Jeśli nic nie znajdzie, wypisz debug:
    print(f"DEBUG: Szukałem folderu dla ID {real_bank_id} w: {possible_names}")
    return None
...
def main():
    ...
    # Przykładowa lista banków z lvl17.json: "shape_banks": [1, 2, 9, 20]
    # W prawdziwym systemie powinieneś to wczytywać dynamicznie dla każdego poziomu.
    active_banks_context = [1, 2, 9, 20] 
